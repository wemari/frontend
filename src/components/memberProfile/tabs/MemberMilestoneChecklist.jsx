...
export default function MemberMilestoneChecklist({ memberId, onAddClick, refresh }) {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const [r, t] = await Promise.all([ // Load both records and templates simultaneously
  ...
  const handleAdd = async (templateId) => {
    ...
    try {
      ...
      if (templateStillValid) {
        ...
        await load(); // Fallback if the template is no longer in the valid list
      }
              ...
              return (
                <Tooltip key={t.id} title={tooltip}>
                  <Chip
                    ...
                    deleteIcon={completed && canDeleteMilestone ? <Trash size={16} /> : undefined}
                  />
                </Tooltip>
              );
            })}
